...
import { openMapConfig } from "./config";
...
export default function OpenTransitMap({ latitude, longitude, address, nearby, route }: TransitMapProps) {
    ...
    const map = new maplibregl.Map({
      ...
      style: {
        ...
        sources: {
          "ratroo-osm": {
            type: "raster",
            tiles: [openMapConfig.rasterTileUrl],
            tileSize: 256,
            attribution: openMapConfig.attribution,
          },
        },
        layers: [
  ...
  return (
    <div className="transit-map-shell">
      <div ref={containerRef} className="transit-map" aria-label="Interactive map showing nearby transit and selected routes" />
      ...
      <div className="map-toolbar"><button type="button" onClick={toggle3d} aria-pressed={pitched}>{pitched ? "2D view" : "3D view"}</button><span>Open map · live Ratroo data</span></div>
