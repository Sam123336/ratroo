...
export default function RiderPortal() {
  ...
  async function saveRoute(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const fixedCommunityService = ["AUTO", "E_RICKSHAW", "SHARED_TAXI"].includes(vehicleType);
      if (fixedCommunityService && stops.some(stop => stop.latitude == null || stop.longitude == null)) {
        throw new Error("Please choose every stop on the map. This lets riders near the route discover your service anywhere in India.");
      }
      const body = {
        name: String(form.get("name")), vehicleType,
        vehicleId: String(form.get("vehicleId") || "") || undefined,
        fareINR: form.get("fareINR") ? Number(form.get("fareINR")) : undefined,
        operatingDays: form.getAll("operatingDays").map(Number),
    ...
    <main className="rider-shell">
      ...
      <section className="rider-workspace">
        ...
        <div className="rider-form-card">
          ...
          {step === 3 && <form onSubmit={saveRoute}>
            <h2>Add the stops you serve</h2>
            <p className="review-explainer">Search a stop or choose it on the map—you do not need to be standing there. Ratroo detects its Indian state automatically.</p>
            <label>Route name<input name="name" required minLength={3} placeholder="Example: Garia to Baruipur" /></label>
            <div className="two"><label>Vehicle type<select name="vehicleType" required defaultValue={vehicles[0]?.vehicleType || "BUS"}>{["BUS","MINIBUS","AUTO","E_RICKSHAW","SHARED_TAXI"].map(value => <option key={value}>{value}</option>)}</select></label><label>Use vehicle<select name="vehicleId" defaultValue={vehicles[0]?.id || ""}><option value="">No fixed vehicle</option>{vehicles.map(vehicle => <option value={vehicle.id} key={vehicle.id}>{vehicle.registrationNumber}</option>)}</select></label></div>
            <div className="stops-editor"><div className="stops-title"><h3>Stops in driving order</h3><span>{stops.length} stops</span></div>{stops.map((stop,index) => <div className="stop-row" key={index}><span>{index + 1}</span><StopSearchInput index={index} stop={stop} label={index === 0 ? "Starting stand / stop" : index === stops.length - 1 ? "Last stop" : "Next stop"} placeholder={index === 0 ? "Search where you normally start" : "Search stop name"} onChange={value => setStops(rows => rows.map((row,rowIndex) => rowIndex === index ? { ...row, stopName: value, latitude: undefined, longitude: undefined } : row))} onSelect={suggestion => { setStops(rows => rows.map((row,rowIndex) => rowIndex === index ? { ...row, stopName: suggestion.name, latitude: suggestion.latitude, longitude: suggestion.longitude } : row)); setMessage(suggestion.latitude != null && suggestion.longitude != null ? `${suggestion.name} selected. Check the pin on the map if needed.` : `${suggestion.name} selected. Choose its exact point on the map.`); }} /><button type="button" className={stop.latitude != null ? "located" : "pin"} onClick={() => setMapStopIndex(index)} title="Choose this stop on the map">{stop.latitude != null ? "✓ Map" : "⌖ Map"}</button>{stops.length > 2 && <button type="button" className="remove" onClick={() => setStops(rows => rows.filter((_,rowIndex) => rowIndex !== index))}>×</button>}</div>)}<button type="button" className="add-stop" onClick={() => setStops(rows => [...rows, { stopName: "" }])}>＋ Add another stop</button></div>
            <fieldset><legend>Days you normally run</legend><div className="service-days">{["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((day,index) => <label key={day}><input type="checkbox" name="operatingDays" value={index + 1} defaultChecked /><span>{day}</span></label>)}</div></fieldset>
            <div className="two"><label>Full route fare (₹)<input name="fareINR" type="number" min={0} inputMode="numeric" placeholder="Optional" /></label><label>Extra note<input name="notes" placeholder="Example: Every 30 minutes" /></label></div>
            <button className="primary" disabled={busy}>Save this route →</button>
          </form>}
